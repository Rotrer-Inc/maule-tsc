	...
	*/
	...
	if($registroWebPay){
		...
		if( ($datosFolio->final - $datosFolio->actual) <= $datosFolio->alerta){
				...
				$mailFolios->Host = 'mail.intervialchile.cl';
				$mailFolios->SMTPAuth = true;
				$mailFolios->Username = 'tsc.maule@intervialchile.cl';
				$mailFolios->Password = 'tscm4ul3';
				$mailFolios->FromName = 'WebSite Ruta del Maule';
				$mailFolios->From = 'tsc@intervialchile.cl';
		...
		if($creaArchivoPago =  fopen($fullPathArchivoPagoWsTemp, "w")){
			...
			if(fwrite($creaArchivoPago, $datosParaArchivo)) {
				...
				$mail->Password = 'tscm4ul3';
				$mail->FromName = 'WebSite Ruta del Maule';
				$mail->From = 'tsc@intervialchile.cl';
				...
				*/
				...
				wp_mail( EMAIL_MAULE, 'WebSite Ruta del Maule', $bodynew, $headers );
		...
		if(isset($datosTac->emailRecep) && !empty($datosTac->emailRecep)){
			...
			*/
			...
			$emailRecep = "cristian@rotrer.com";
			$headers = "MIME-Version: 1.0";
			add_filter( 'wp_mail_content_type', 'set_html_content_type' );
...
function yoursite_wp_mail_from($content_type) {
	return 'tsc@intervialchile.cl';
}
add_filter('wp_mail_from_name','yoursite_wp_mail_from_name');
function yoursite_wp_mail_from_name($name) {
	return 'WebSite Ruta del Maule';
}
